# ============================================
# TEST RAPIDO - Sin N8N (Solo Backend)
# ============================================

Write-Host "`nTEST RAPIDO - Sistema de Planificaciones" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "   OK Backend respondiendo`n" -ForegroundColor Green
} catch {
...
# 2. Login
Write-Host "2. Obteniendo token..." -ForegroundColor Yellow
try {
    $loginData = @{
    ...
    Write-Host "   OK Token obtenido: $($token.Substring(0, 20))...`n" -ForegroundColor Green
} catch {
...
# 3. Verificar aulas
Write-Host "3. Verificando aulas disponibles..." -ForegroundColor Yellow
try {
    ...
    Write-Host "   OK $aulasCount aulas disponibles`n" -ForegroundColor Green
    
    if ($aulasCount -eq 0) {
        Write-Host "   ADVERTENCIA: No hay aulas en la BD`n" -ForegroundColor Yellow
    }
} catch {
    Write-Host "   ADVERTENCIA: No se pudo obtener aulas`n" -ForegroundColor Yellow
}

# 4. Verificar endpoint de planificaciones
Write-Host "4. Verificando endpoint de planificaciones..." -ForegroundColor Yellow
try {
    $estadoResponse = Invoke-RestMethod -Uri "http://localhost:3000/api/planificaciones/distribucion/1" `
        -Method Get `
        -Headers @{Authorization = "Bearer $token"}
    
    Write-Host "   OK Endpoint de planificaciones funcionando" -ForegroundColor Green
    Write-Host "   Estadisticas actuales:" -ForegroundColor Cyan
...
Write-Host "COMO PROBAR LA SUBIDA DE PLANIFICACION:`n" -ForegroundColor Cyan
...
Write-Host "   OK Backend funcionando (puerto 3000)" -ForegroundColor Green
Write-Host "   OK Base de datos conectada" -ForegroundColor Green
Write-Host "   OK Autenticacion funcionando" -ForegroundColor Green
Write-Host "   OK Endpoint de planificaciones activo" -ForegroundColor Green
Write-Host "   OK Sistema de roles configurado" -ForegroundColor Green
Write-Host "   PENDIENTE N8N workflow inactivo" -ForegroundColor Yellow
Write-Host ""

Write-Host "RESULTADO:" -ForegroundColor Cyan
Write-Host "   • Puedes subir planificaciones: SI" -ForegroundColor Green
Write-Host "   • Se guardan en la base de datos: SI" -ForegroundColor Green
Write-Host "   • Distribucion sera MANUAL (sin n8n)" -ForegroundColor Yellow
Write-Host ""

Write-Host "SIGUIENTE PASO:" -ForegroundColor Cyan
Write-Host "   -> Crea el Excel y prueba la subida con Postman" -ForegroundColor White
Write-Host "   -> Luego decide si activar N8N para distribucion automatica`n" -ForegroundColor White

Write-Host "AYUDA:" -ForegroundColor Cyan
Write-Host "   • Ver guia completa: CREAR_EXCEL_PRUEBA_MANUAL.md" -ForegroundColor Gray
Write-Host "   • Activar N8N despues: ACTIVAR_N8N_PASO_A_PASO.md`n" -ForegroundColor Gray
